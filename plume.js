...
window.PLUME_REGLES = [
  ...
  { id: 'verylot', sev: 'pauvre', re: /\b(?!(?:the|this|that|these|those|its|his|her|their|our|my|your)\b)\w+\s+very\b|\b(?:really|a\s+lot\s+of|lots\s+of|totally|completely\s+\w+ly)\b/gi, t: 'Intensifieur pauvre', d: "« very important » → <b>crucial</b>, « a lot of » → <b>a great deal of</b>, <b>numerous</b>, <b>considerable</b>. Un adjectif précis vaut mieux qu'un adverbe d'intensité.<br>À ne pas confondre avec le <b>very emphatique</b> — « the very problem it seeks to solve », « the very idea », « this very day » — qui est du bon anglais et n'est plus signalé." },
  ...
  /* --- tirées de sa copie IELTS du 14/09/2026 (« platforms, not publishers »).
         Plusieurs sont des RÉCURRENCES de l'essai du 12/09 : le relecteur
         doit les attraper avant moi, sinon il les refait à chaque copie. --- */
  { id: 'accordcollectif', sev: 'faute', re: /\bsocial\s+media\s+(?!companies|firms|platforms|giants|sites|groups|services|executives)(?:[\w']+\s+){0,5}?their\b/gi, t: '« social media … their »', d: "<b>social media</b> est aujourd'hui un indénombrable singulier : <i>social media IS</i>, et donc <b>ITS</b> platforms. Si tu penses aux entreprises — et c'est presque toujours le cas — écris <b>social media companies</b> / <b>the platforms</b> : là, <i>they</i> et <i>their</i> sont justes. L'erreur n'est pas le pronom, c'est le sujet." },
  { id: 'suchsing', sev: 'faute', re: /\bsuch\s+(?:measure|decision|statement|argument|policy|approach|change|system|proposal|reform)\b/gi, t: '« such measure »', d: "Devant un nom singulier dénombrable : <b>such A measure</b>. Sans article, « such » ne va qu'avec un pluriel ou un indénombrable (<i>such measures</i>, <i>such evidence</i>)." },
  { id: 'endupbase', sev: 'faute', re: /\bend(?:s|ed|ing)?\s+up\s+(?:worsen|make|do|be|have|solve|create|cost|take|give|become|pay|lose|look|feel)\b/gi, t: '« end up + base verbale »', d: "<b>end up + -ING</b> : <i>might end up worsening it</i>. Même moule que <i>give up</i>, <i>keep on</i>, <i>be worth</i>." },
  { id: 'awaitfor', sev: 'faute', re: /\bawait(?:s|ing|ed)?\s+for\b/gi, t: '« await for »', d: "<b>await</b> se construit sans préposition (<i>await a decision</i>) ; c'est <b>wait FOR</b> qui prend « for ». Les deux ne se mélangent jamais." },
  { id: 'pavepath', sev: 'faute', re: /\bpaves?\s+the\s+path\b/gi, t: '« pave the path »', d: "Collocation figée : on <b>paves the WAY</b> for something. « Path » va avec <i>clear a path</i>, <i>beat a path</i>." },
  { id: 'applieslogic', sev: 'faute', re: /\b(?:logic|reasoning|argument|principle|rule)\s+(?:applies|apply|applied)\s+for\b/gi, t: '« the logic applies for »', d: "<b>apply TO</b> something. « Apply for » = postuler à, faire une demande de." },
  { id: 'transpirefrom', sev: 'calque', re: /\btranspire[sd]?\s+(?:from|through|in)\b/gi, t: 'faux ami « transpire »', d: "<b>transpire</b> = s'avérer, se révéler (<i>it transpired that…</i>). « Transparaître » se dit <b>show through</b>, ou se contourne : <b>the intention behind such a measure</b>, <b>what the measure betrays</b>." },
