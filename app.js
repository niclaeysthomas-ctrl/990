...
function load() {
  try {
    const raw = JSON.parse(localStorage.getItem('t990') || '{}');
...
function renderHome() {
  ...
  const placementBanner = !S.placementDone ? `
    <div class="banner">
      ...
      <div class="d">Un test complet (écoute + lecture + grammaire) pour estimer ton score TOEIC actuel et démarrer ta courbe de progression.</div>
  ...
  const scoreCard = S.estScore != null ? `
    <button class="tile" onclick="setView('exam')">
      <div class="ic e">📈</div>
      <div class="body"><div class="t">Score TOEIC estimé</div><div class="d">${scoreBand(S.estScore)} · ${S.history.length} test(s) passé(s)</div></div>
      <div class="badge">${S.estScore}</div>
    </button>` : '';
  ...
  app.innerHTML = `
    ...
    <button class="tile" onclick="setView('traduire')">
      ...
      <div class="body"><div class="t">Traduire (FR ⇄ EN)</div><div class="d">Phrases + correction idiomatique & grammaire</div></div>
...
function renderAnkiHome() {
  ...
  app.innerHTML = `
    <div class="card">
      ...
      <div class="sub">Répétition espacée SM‑2, sens tiré <b style="color:var(--txt)">au hasard</b> 🇬🇧→🇫🇷 ou 🇫🇷→🇬🇧 : tu dois savoir <i>produire</i> le mot, pas juste le reconnaître. Objectif : maîtrise C2, du quotidien aux idiomes.</div>
      <div class="row2 mt">
...
function renderVocabBrowser(query) {
  app.innerHTML = `
    <div class="card">
      ...
      <div class="sub">Cherche en français ou en anglais (ou par thème). Touche 🔊 pour écouter la prononciation.</div>
...
function renderCard() {
  ...
  // Sens tiré au hasard : produire l'anglais OU le français
  ...
  app.innerHTML = `
    ...
    <div class="flash" onclick="flip()">
      ...
      <div id="taphint" class="tap">${R.dir === 'en2fr' ? 'Traduis en français' : 'Traduis en anglais'} · touche pour révéler ▽</div>
    </div>
    <button id="spk" class="spk ${R.dir === 'fr2en' ? 'hidden' : ''}" onclick="speak('${enEsc}');event.stopPropagation()" title="Écouter l'anglais">🔊</button>
    <div id="ratebar"></div>
  `;
}
function flip() {
  ...
  const i = R.queue[R.pos];
  if (R.dir === 'fr2en') {                 // l'anglais était caché : on le révèle et on le prononce
...
function pickVoice() {
  ...
  voiceEN = en.find(v => /en-US/i.test(v.lang) && /female|Samantha|Zira|Google US/i.test(v.name))
         || en.find(v => /en-US/i.test(v.lang)) || en[0] || null;
  voiceW = en.find(v => /female|Samantha|Victoria|Karen|Zira|Susan|Fiona|Moira/i.test(v.name)) || voiceEN;
  voiceM = en.find(v => /male|Daniel|Alex|Fred|David|Aaron|Oliver|Arthur/i.test(v.name)) || voiceEN;
}
...
function speak(text) {
  ...
  u.lang = 'en-US'; u.rate = audioRate(0.95); u.pitch = 1;
...
/* Conversion score : proportion correcte -> score de section TOEIC (5-495) */
function sectionScore(p) {
  return Math.max(5, Math.min(495, Math.round((5 + 490 * p) / 5) * 5));
}
function scoreBand(total) {
  if (total >= 900) return 'Niveau visé ESCP atteint 🎯';
  if (total >= 785) return 'Autonome (C1) — proche de la cible';
  if (total >= 600) return 'Intermédiaire+ (B2)';
  if (total >= 470) return 'Intermédiaire (B1)';
  if (total >= 300) return 'Élémentaire (A2)';
  return 'Débutant (A1)';
}
function levelFromScore(total) {
  if (total >= 785) return 3;
...
function finishExam() {
  ...
  const Lp = lt ? lc / lt : 0, Rp = rt ? rc / rt : 0;
  const Lsc = sectionScore(Lp), Rsc = sectionScore(Rp), total = Lsc + Rsc;

  const rec = { ts: Date.now(), date: todayStr(), total, L: Lsc, R: Rsc, kind: EX.kind, diff: EX.diff, mins,
                lc, lt, rc, rt };
  S.history.push(rec);
  S.estScore = total;
  ...
  app.innerHTML = `
    <div class="card big">
      <div class="em">${total >= 900 ? '🏆' : total >= 785 ? '🎉' : '📊'}</div>
      <div class="score">${total}<span style="font-size:20px;color:var(--muted)"> /990</span></div>
      <div class="lab">${scoreBand(total)}</div>
      ${deltaHtml}
    </div>
    <div class="card">
      <div class="scoreline">
        <div><div class="v" style="color:var(--purple)">${Lsc}</div><div class="k">Écoute /495</div></div>
        <div><div class="v" style="color:var(--blue)">${Rsc}</div><div class="k">Lecture /495</div></div>
        <div><div class="v">${mins}′</div><div class="k">durée</div></div>
      </div>
      <div class="sub center">Écoute ${lc}/${lt} · Lecture ${rc}/${rt} bonnes réponses</div>
    </div>
...
function progressChart() {
  ...
  const min = Math.min(400, Math.min(...pts) - 40);
  const max = Math.max(990, Math.max(...pts) + 40);
  ...
  // ligne cible 900
  const yTarget = y(900);
  const targetLine = (900 >= min && 900 <= max)
    ? `<line x1="${pad}" y1="${yTarget}" x2="${W - pad}" y2="${yTarget}" stroke="var(--good)" stroke-dasharray="4 4" stroke-width="1" opacity=".6"/>
       <text x="${W - pad}" y="${yTarget - 4}" fill="var(--good)" font-size="9" text-anchor="end">cible 900</text>` : '';
  return `
...
function renderExamHome() {
  if (!S.placementDone) {
    app.innerHTML = `
      <div class="card">
        ...
        <div class="sub">Avant tout : passe un test complet pour estimer ton score TOEIC de départ. Il combine écoute, grammaire et lecture, et démarre ta courbe de progression.</div>
      </div>
      <button class="btn" onclick="startExam('placement')">Passer le test de niveau (~15 min)</button>
      <div class="sub center mt">20 questions · estimation /990</div>
    `;
    return;
  }
  const cur = S.level;
  ...
  app.innerHTML = `
    <div class="card">
      <div class="sub" style="text-transform:uppercase;letter-spacing:.06em;font-size:11px;font-weight:800">Score TOEIC estimé</div>
      <div style="display:flex;align-items:baseline;gap:10px"><div class="logo" style="font-size:40px">${S.estScore}</div><div class="sub">/990</div></div>
    ...
    <div class="card">
      ...
      <div class="sub">40 questions chronométrées (écoute + lecture). Choisis la difficulté :</div>
...
/* ============================================================
   TRADUIRE — version FR ⇄ EN, correction idiomatique + grammaire
   ============================================================ */
...
function renderTransHome() {
  ...
  const dir = S.transDir === 'en2fr' ? 'EN → FR' : 'FR → EN';
  app.innerHTML = `
    <div class="card">
...
function renderTransCard() {
  ...
  app.innerHTML = `
    ...
    <textarea id="trans-in" class="transinput" rows="2" placeholder="${en2fr ? 'Écris ta traduction en français' : 'Écris ta traduction en anglais'} (facultatif)…"></textarea>
...
function revealTrans() {
  ...
  app.innerHTML = `
    ...
    <div class="trans-model">
      <div class="lbl">Correction idiomatique <button class="spk sm" onclick="speak('${enEsc}')" title="Écouter l'anglais">🔊</button></div>
...
/* ============================================================
   ANGLAIS SOUTENU — reformuler du neutre vers l'élégant (EN → EN)
   ============================================================ */
...
function finishReg() {
  ...
  app.innerHTML = `
    <div class="card big">
      ...
      <div class="mt sub">Un registre soutenu se construit tournure par tournure. C'est la marque d'un anglais vraiment maîtrisé.</div>
    </div>
